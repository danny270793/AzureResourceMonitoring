...
import { Resource, ResourceGroup } from '@danny270793/azureservicesclient/build/responses'
import Kubernetes from '@danny270793/azureservicesclient/build/responses/kubernetes'
import virtualMachineInstanceView, { virtualMachineInstanceViewStatus } from '@danny270793/azureservicesclient/build/responses/vm-instance-view'
import Telegram from './libraries/telegram';

async function main(): Promise<void> {
    const azureSubscriptionsNames: string[] = (process.env.AZURE_SUBSCRIPTIONS_NAMES || '').split(',')
    const azureSubscriptionsIds: string[] = (process.env.AZURE_SUBSCRIPTIONS_IDS || '').split(',')
